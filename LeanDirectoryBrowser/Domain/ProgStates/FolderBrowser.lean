...
def getFolderBrowserCodeList (ps : ProgState) (_: isProgStateFolderBrowser ps) : List Code :=
  match ps with
...
def processFolderBrowser (ps : ProgState) (_ : isProgStateFolderBrowser ps) (input : String) : ProgState :=
  match ps with
    | ProgState.folderBrowser root hRootIsDir currentDirectory hCurrentDirIsNonEmptyDir displayWidth displayHeight displayRows displayColumns displayColumnWidth selectedFilePath fileOnTopPath runPowerShell =>
      let folderBrowserPs := ProgState.folderBrowser root hRootIsDir currentDirectory hCurrentDirIsNonEmptyDir displayWidth displayHeight displayRows displayColumns displayColumnWidth selectedFilePath fileOnTopPath runPowerShell
      match input with
...
def withPowerShellStartedInFolderBrowser (cpp : CodeProxyProcess) (ps : ProgState) (_: isProgStateFolderBrowser ps) : IO ProgState := do
    match ps with
