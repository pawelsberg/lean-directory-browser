...
namespace EmptyFolderBrowser
  def getCodeList (ps : ProgState) (_: isProgStateEmptyFolderBrowser ps) : List Code :=
    match ps with
  ...
  def process (ps : ProgState) (_ : isProgStateEmptyFolderBrowser ps) (input : String) : ProgState :=
    match ps with
  ...
  def withPowerShellStarted (cpp : CodeProxyProcess) (ps : ProgState) (_: isProgStateEmptyFolderBrowser ps) : IO ProgState := do
      match ps with
